...
export default async function ProfilePage() {
  ...
  return (
    <main className="min-h-screen bg-[#0d1216] text-white">
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute left-1/2 top-[-320px] h-[620px] w-[620px] -translate-x-1/2 rounded-full bg-cyan-500/[0.05] blur-[150px]" />
        <div className="absolute right-[-160px] top-[38%] h-[420px] w-[420px] rounded-full bg-purple-500/[0.04] blur-[140px]" />
      </div>

      <header className="relative z-10 border-b border-white/[0.07] bg-[#172129]/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <Link href="/mangas" className="group flex items-center gap-3 text-sm text-gray-400 transition hover:text-white">
            <span className="flex h-9 w-9 items-center justify-center rounded-xl border border-white/10 bg-white/[0.03] transition group-hover:border-cyan-400/50"><ArrowLeft size={17} /></span>
            <span className="hidden sm:inline">Voltar para coleção</span>
          </Link>
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-gray-400"><UserRound size={15} className="text-cyan-400" /> Perfil</div>
        </div>
      </header>

      <section className="relative z-10 border-b border-white/[0.06] bg-[#172129]">
          ...
          <div className="flex min-w-0 items-center gap-4 sm:gap-5">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full border border-cyan-400/25 bg-cyan-400/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-cyan-300">Colecionador</span>
              </div>
              <p className="mt-1 truncate text-sm text-gray-400">{email}</p>
              <p className="mt-3 flex items-center gap-2 text-xs text-gray-500"><Activity size={13} className="text-cyan-400" /> Construindo uma coleção de histórias</p>
            </div>
          </div>
      ...
      <nav className="relative z-10 border-b border-white/[0.07] bg-[#11191f]">
        <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-4 py-2 scrollbar-none sm:px-6">
          ...
          ].map(([label, href, active]) => <a key={label as string} href={href as string} className={`shrink-0 border-b-2 px-3 py-3 text-xs font-semibold transition ${active ? 'border-cyan-400 text-white' : 'border-transparent text-gray-500 hover:border-white/20 hover:text-gray-200'}`}>{label as string}</a>)}
        </div>
      </nav>
        ...
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_290px]">
          <div className="min-w-0">
            <section className="border-b border-white/[0.08] pb-8">
              ...
              <div className="mt-5 rounded-2xl border border-white/[0.08] bg-[#151f26] p-4 sm:p-5">
                <div className="mb-4 flex items-center gap-2 text-xs text-gray-400"><Pencil size={13} className="text-cyan-400" /> Escolha as obras que representam sua coleção</div>
            ...
            <section id="atividade" className="scroll-mt-20 border-b border-white/[0.08] py-8">
              ...
              {mangas.length === 0 ? <EmptyState text="As atualizações da sua coleção aparecerão aqui." /> : <div className="mt-5 divide-y divide-white/[0.07] rounded-2xl border border-white/[0.08] bg-[#121a20]">{mangas.slice(0, 6).map((manga) => <ActivityRow key={manga.id} manga={manga} />)}</div>}
            </section>

            <section id="colecao" className="scroll-mt-20 border-b border-white/[0.08] py-8">
              <SectionHeading eyebrow="Acervo pessoal" title="Minha coleção" action={<Link href="/mangas" className="text-xs font-semibold text-cyan-400 transition hover:text-cyan-300">Ver coleção completa</Link>} />
          ...
          <aside className="space-y-5 lg:sticky lg:top-5 lg:self-start">
            <section id="estatisticas" className="scroll-mt-20 rounded-2xl border border-white/[0.08] bg-[#151f26] p-5">
              <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Star size={16} className="text-cyan-400" /><h2 className="font-semibold">Suas avaliações</h2></div><span className="text-xs text-gray-500">{rated.length}</span></div>
              <div className="mt-5 flex items-end justify-between gap-4"><div><p className="text-3xl font-bold text-white">{averageNote ? averageNote.toFixed(1) : '—'}</p><p className="mt-1 text-xs text-gray-500">média das notas</p></div><div className="flex items-center gap-1 text-cyan-300">{[1, 2, 3, 4, 5].map((item) => <Star key={item} size={14} fill={item <= Math.round(averageNote) ? 'currentColor' : 'none'} />)}</div></div>
              <div className="mt-6 space-y-2">{ratingBars.map((item) => <div key={item.rating} className="flex items-center gap-2 text-[11px] text-gray-500"><span className="w-3">{item.rating}</span><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-gray-800"><div className="h-full rounded-full bg-cyan-400" style={{ width: `${(item.count / maxRatingCount) * 100}%` }} /></div><span className="w-4 text-right">{item.count}</span></div>)}</div>
            </section>

            <section className="rounded-2xl border border-white/[0.08] bg-[#151f26] p-5">
              <div className="flex items-center gap-2"><Clock3 size={16} className="text-cyan-400" /><h2 className="font-semibold">Resumo</h2></div>
            ...
            <section className="rounded-2xl border border-cyan-400/15 bg-cyan-400/[0.04] p-5"><div className="flex items-center gap-2 text-cyan-300"><MessageCircle size={16} /><h2 className="font-semibold">Continue explorando</h2></div><p className="mt-3 text-sm leading-6 text-gray-400">Encontre novas obras para acompanhar e deixe sua coleção cada vez mais com a sua cara.</p><Link href="/mangas" className="mt-4 inline-flex text-xs font-bold uppercase tracking-wider text-cyan-300 transition hover:text-white">Explorar catálogo →</Link></section>
          </aside>
        </div>
      </div>
    </main>
...
function SectionHeading({ eyebrow, title, action }: { eyebrow: string; title: string; action?: React.ReactNode }) {
  return <div className="flex items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-cyan-400">{eyebrow}</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-white">{title}</h2></div>{action}</div>
}

function ProfileStat({ icon, value, label }: { icon: React.ReactNode; value: number; label: string }) {
  return <div className="rounded-xl border border-white/[0.08] bg-black/10 px-3 py-2.5 text-center"><div className="mx-auto flex w-fit text-cyan-400">{icon}</div><p className="mt-1 text-xl font-bold text-white">{value}</p><p className="text-[10px] uppercase tracking-wider text-gray-500">{label}</p></div>
}
...
function WorkCard({ manga, rank }: { manga: { id: string; name: string; author: string | null; coverUrl: string | null; note: number | null; ownedVolumes: number[]; totalVolumes: number | null }; rank: number }) {
  return <Link href={`/mangas/${manga.id}`} className="group overflow-hidden rounded-2xl border border-white/[0.08] bg-[#151f26] transition hover:-translate-y-1 hover:border-cyan-400/40"><div className="relative aspect-[3/4] bg-gray-800">{manga.coverUrl ? <Image src={manga.coverUrl} alt={`Capa de ${manga.name}`} fill sizes="(max-width: 640px) 45vw, 220px" className="object-cover transition duration-300 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-sm text-gray-500">Sem capa</div>}<span className="absolute left-2 top-2 rounded-md bg-[#0d1216]/90 px-2 py-1 text-[10px] font-bold text-cyan-300">#{rank}</span></div><div className="p-3"><h3 className="truncate text-sm font-semibold text-white">{manga.name}</h3><p className="mt-1 truncate text-xs text-gray-500">{manga.author || 'Autor não informado'}</p><div className="mt-3 flex items-center justify-between text-xs"><span className="text-gray-400">{manga.ownedVolumes.length}{manga.totalVolumes ? `/${manga.totalVolumes}` : ''} vol.</span>{manga.note !== null && <span className="flex items-center gap-1 text-cyan-300"><Star size={12} fill="currentColor" />{manga.note.toFixed(1)}</span>}</div></div></Link>
}
...
function CollectionRow({ manga, showMissingVolumes = false }: { manga: { id: string; name: string; coverUrl: string | null; status: string | null; collectionType: string; ownedVolumes: number[]; totalVolumes: number | null; updatedAt: Date }; showMissingVolumes?: boolean }) {
  ...
  return <div className="rounded-xl border border-white/[0.08] bg-[#121a20] p-3 transition hover:border-cyan-400/25 hover:bg-[#172129]"><Link href={`/mangas/${manga.id}`} className="flex items-center gap-3"><div className="relative h-14 w-10 shrink-0 overflow-hidden rounded-md bg-gray-800">{manga.coverUrl && <Image src={manga.coverUrl} alt="" fill sizes="40px" className="object-cover" />}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h3 className="truncate text-sm font-semibold text-white">{manga.name}</h3><span className="shrink-0 text-[10px] uppercase tracking-wider text-cyan-300">{manga.collectionType === 'HQ' ? 'HQ' : 'Mangá'}</span></div><p className="mt-1 text-xs text-gray-500">{statusLabel(manga.status)} · {manga.ownedVolumes.length}{manga.totalVolumes ? `/${manga.totalVolumes}` : ''} volumes</p>{progress !== null && <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-gray-800"><div className="h-full rounded-full bg-cyan-400" style={{ width: `${progress}%` }} /></div>}</div></Link>{showMissingVolumes && <div className="mt-2 border-t border-white/[0.06] pt-2">{missingVolumes === null ? <p className="text-[11px] text-gray-500">Total de volumes não informado</p> : missingVolumes.length === 0 ? <p className="text-[11px] text-emerald-300">Coleção completa</p> : <details><summary className="cursor-pointer list-none text-[11px] font-medium text-amber-300">Faltam {missingVolumes.length} {missingVolumes.length === 1 ? 'volume' : 'volumes'}</summary><p className="mt-1 text-[11px] leading-5 text-gray-400">Volumes: {missingVolumes.join(', ')}</p></details>}</div>}</div>
}

function EmptyState({ text }: { text: string }) {
  return <div className="mt-5 rounded-xl border border-dashed border-white/[0.12] bg-[#121a20] p-8 text-center text-sm text-gray-500">{text}</div>
}
